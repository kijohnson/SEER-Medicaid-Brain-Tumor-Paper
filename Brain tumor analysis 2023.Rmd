...
seer0to39 <- seer0to39 %>%
  mutate(brain = case_when (AYA_site_recode_WHO_2008 %in% c(7,8,9) ~ 0, # 3.1 Astrocytoma
                            ...
                            AYA_site_recode_WHO_2008 %in% c(14) ~ 4, # 3.5 Other specified intracranial and intraspinal neoplasms
...
# combine because of small numbers (3.5 Other specified intracranial and intraspinal neoplasms, 3.6 Unspecified intracranial and intraspinal neoplasms)
seer0to39 <- seer0to39 %>%
...
seer0to39$brain2 <- factor(seer0to39$brain2, levels = c(0:4), labels = c(
  ...
  "3.3 Ependymoma",
  "3.4 Medulloblastoma and other PNET",
  "Other specified (3.5) and unspecified (3.6) intracranial and intraspinal neoplasms"))
...
table(seer0to39$brain2, seer0to39$Grade)
```
...
seer0to39_stage <- seer0to39 %>%
  filter(!is.na(stage)) %>% 
  select(stage, In_Medicaid_Flag, SEER_registry, mcont66.2, race_ethnicity2, race_cat2, age_cat, age, Yost_ACS_2006_2010_quintile, state, brain, brain2, age_cat2, age_cat3, Year_of_diagnosis) %>%
...
# get stage model n's  for exposure x outcome for revision for Table 2
```{r}
# Filter the dataset
filtered_data <- seer0to39_stage %>% filter(age_cat3 == "0 to 14")
filtered_data2 <- seer0to39_stage %>% filter(age_cat3 == "15 to 39")

# Get the total number of observations
n_total <- nrow(filtered_data)

table(filtered_data$stage2, filtered_data$In_Medicaid_Flag)
table(filtered_data$stage2, filtered_data$mcont66.2)

# Get the total number of observations
n_total <- nrow(filtered_data2)

table(filtered_data2$stage2, filtered_data2$In_Medicaid_Flag)
table(filtered_data2$stage2, filtered_data2$mcont66.2)
```
...
plot_KM_curves <- function(myvar, agevar, labs, title) {
  ...
  # Filter the dataset based on age category
  x <<- seer0to39 %>% filter(age_cat3 == agevar)
  
  # Wrap legend labels
  wrapped_labs <- str_wrap(labs, width = 10)  # Adjust the width as needed
  
  # Fit the survival model and create the plot
  p <- ggsurvplot(
    fit = survfit(as.formula(formula_str), data = x), 
    size = 0.75,
    ...
    legend.labs = wrapped_labs,
    linetype = "strata",
    palette = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB"),
    font.x = c(12, "bold"),
    font.y = c(12, "bold"),
    xlim = c(0, 60),
    break.x.by = 12,
...
plot3 <- plot_KM_curves("mcont66.2", agevar = "0 to 14", c("Not enrolled", "Continuous", "Other discontinuous", "Other", "At diagnosis"), title = "C. 0 to 14") 
plot4 <- plot_KM_curves("mcont66.2", agevar = "15 to 39", c("Not enrolled", "Continuous", "Other discontinuous", "Other", "At diagnosis"), title = "D. 15 to 39") 
...
plot3a <- plot_KM_curves("mcont66.2", agevar = "0 to 14", c("Not enrolled", "Continuous", "Other discontinuous", "Other", "At diagnosis"), title = "C. 0 to 14") 
plot4a <- plot_KM_curves("mcont66.2", agevar = "15 to 39", c("Not enrolled", "Continuous", "Other discontinuous", "Other", "At diagnosis"), title = "D. 15 to 39") 
...
SuppTable3 <- SuppTable3 %>%
  mutate(abbrev = case_when(Brain_tumor_subtype == "3.1 Astrocytoma" ~ 0,
                            ...
                            Brain_tumor_subtype == "Other specified (3.5) and unspecified (3.6) intracranial and intraspinal neoplasms" ~ 4))
...
Plot1 <- SuppTable3 %>%
  ...
  theme_bw() +
  theme(axis.line = element_line(colour = c("black", "#D2D2D2", "#A0A0A0", "#6E6E6E", "#EBEBEB")),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_blank(),
    panel.background = element_blank(),
    ...
    axis.title.y=element_text(size=12, face = "bold", color = "black"))+
  coord_flip() +
  labs(color = NULL, x= "CNS tumor subtype", y = "60-month survival (%)", title = "A. 0 to 14 years") +
  ylim(0, 100) +
...
Plot2 <- SuppTable3 %>%
  ...
  geom_errorbar(aes(y = Survival_Percentage, x = abbrev, ymax = LowerCI, ymin = UpperCI, group = Category, color = Category), height =0.5, width = 0.4, position = position_dodge(width=0.9)) +
  theme_bw() +
  theme(axis.line = element_line(colour =c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_blank(),
    panel.background = element_blank(),
    ...
    axis.title.y=element_text(size=12, face = "bold", color = "black"))+
  coord_flip() +
  labs(color = NULL, x= "CNS tumor subtype", y = "60-month survival (%)", title = "B. 15 to 39 years") +
  ylim(0, 100) +
  scale_color_manual(values = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")) + 
  theme(strip.placement = "inside", legend.position = "bottom",  strip.text = element_text(size = 9, face="bold", color="black"), strip.background = element_rect(fill="lightgray"), plot.title = element_text(face="bold"), legend.text=element_text(size=12, face="bold")) 
...
Plot3 <- SuppTable3 %>%
  ...
  theme(axis.line = element_line(colour = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")),
    ...
    axis.title.y=element_text(size=12, face = "bold", color = "black"))+
  coord_flip() +
  labs(color = NULL, x= "CNS tumor subtype", y = "60-month survival (%)", title = "C. 0 to 14 years") +
  ylim(0, 100) +
  scale_color_manual(values = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")) + 
  theme(strip.placement = "inside", legend.position = "bottom",  strip.text = element_text(size = 9, face="bold", color="black"), strip.background = element_rect(fill="lightgray"), plot.title = element_text(face="bold"), legend.text=element_text(size=12, face="bold"))  +
...
Plot4 <- SuppTable3 %>%
  ...
  theme(axis.line = element_line(colour = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")),
    ...
    axis.title.y=element_text(size=12, face = "bold", color = "black"))+
  coord_flip() +
  labs(color = NULL, x= "CNS tumor subtype", y = "60-month survival (%)", title = "D. 15 to 39 years") +
  ylim(0, 100) +
  scale_color_manual(values = c("black", "#7D7D7D", "#ABABAB", "#CECECE", "#EBEBEB")) + 
  theme(strip.placement = "inside", legend.position = "bottom",  strip.text = element_text(size = 9, face="bold", color="black"), strip.background = element_rect(fill="lightgray"), plot.title = element_text(face="bold"), legend.text=element_text(size=12, face="bold")) +
   guides(color = guide_legend(nrow = 3, byrow = TRUE))
...
# get survival model n's and persontime for exposure x outcome for revision
```{r}
# Filter the dataset
filtered_data <- seer0to39 %>% filter(age_cat3 == "0 to 14")

filtered_data2 <- seer0to39 %>% filter(age_cat3 == "15 to 39") 

summary_data <- filtered_data  %>%
  group_by(In_Medicaid_Flag) %>%
  summarize(
    events = sum(can_dead),
    total_person_time = sum(fumo),
    incidence_rate = events / total_person_time
  )
irr <- summary_data$incidence_rate[summary_data$In_Medicaid_Flag == "Enrolled"] /
       summary_data$incidence_rate[summary_data$In_Medicaid_Flag == "Not Enrolled"]
...
irr2 <- summary_data$incidence_rate[summary_data$mcont66.2 == "At diagnosis"] /
       summary_data$incidence_rate[summary_data$mcont66.2 == "Not enrolled"]

cox_model <- coxph(Surv(fumo, can_dead) ~ mcont66.2, data = filtered_data)
summary(cox_model)
...
irr3 <- summary_data$incidence_rate[summary_data$In_Medicaid_Flag == "Enrolled"] /
       summary_data$incidence_rate[summary_data$In_Medicaid_Flag == "Not Enrolled"]
...
irr4 <- summary_data$incidence_rate[summary_data$mcont66.2 == "At diagnosis"] /
       summary_data$incidence_rate[summary_data$mcont66.2 == "Not enrolled"]
...
SuppTable5 <- results
SuppTable5$abbrev <- factor(SuppTable5$btype, levels = c("3.1 Astrocytoma", "3.2 Other glioma", "3.3 Ependymoma", "3.4 Medulloblastoma and other PNET", "Other specified (3.5) and unspecified (3.6) intracranial and intraspinal neoplasms"), labels = c("ASTRO", "OGLI", "EPEN", "MB/PNET", "OTHER"))
  ...
  if (nrow(df_subset) > 0) {
    p <- ggplot(df_subset, aes(x = HR, y = Group, color = Category, group=Category)) +
      ...
      theme(legend.position = "bottom",
            ...
            axis.title.y=element_text(size=12, face = "bold", color = "black"),
            plot.title = element_text(face="bold"),
            legend.title = element_text(face = "bold"),  # Title bold
            legend.text = element_text(face = "bold")) +   # Labels bold) 
  theme(legend.title=element_blank()) 
...
C <- plot_survival_results(SuppTable6, "mcont66.2",  title = "C. Race/ethnicity", "race_ethnicity", "Race/ethnicity category")
```
...
# FOR REVISIONS NEURO-ONCOLOGY PRACTICE (NOP)
```{r}
# define year of dx cat (before 2010, ≥2010)
seer0to39_stage_exp <- seer0to39_stage %>%
  mutate(dx_year_cat = case_when(Year_of_diagnosis < 2010 ~ 0,
                                 Year_of_diagnosis >= 2011 ~ 1)) %>%
    filter(state %in% c(6,9,34,53))
  
seer0to39_stage_exp$dx_year_cat <- factor(seer0to39_stage_exp$dx_year_cat, levels = c(0,1), labels = c("diagnosis <2010", "diagnosis ≥ 2011")) %>%



# Adopters since 2010 from: https://www.kff.org/affordable-care-act/issue-brief/states-getting-a-jump-start-on-health/
  #(CA = 6, CT = 9, DC = 11, MN = 27, NJ =34, and WA = 53) 

# get n for each age group before 2010 and 2010 to 2013
summary <- seer0to39_stage_exp %>%
  group_by(age_cat3, stage2, dx_year_cat, In_Medicaid_Flag) %>%
  tally()

# In_Medicaid_Flag adjusted model 0 to 14
model1 <- miceadds::glm.cluster(seer0to39_stage_exp %>% filter(age_cat3 == "0 to 14"), stage2 ~ In_Medicaid_Flag + race_cat2 + age + brain2 + Yost_ACS_2006_2010_quintile + dx_year_cat + In_Medicaid_Flag*dx_year_cat, cluster="state", family="binomial")
summary(model1)
...
# In_Medicaid_Flag adjusted model 15 to 39
model1 <- miceadds::glm.cluster(seer0to39_stage_exp %>% filter(age_cat3 == "15 to 39"), stage2 ~ In_Medicaid_Flag + race_cat2 + age + brain2 + Yost_ACS_2006_2010_quintile + dx_year_cat + In_Medicaid_Flag*dx_year_cat, cluster="state", family="binomial")
summary(model1)
...
# marginally significant for 15 to 39
# In_Medicaid_Flag adjusted model 15 to 39 <2010
model1 <- miceadds::glm.cluster(seer0to39_stage_exp %>% filter(age_cat3 == "15 to 39", Year_of_diagnosis <2010), stage2 ~ In_Medicaid_Flag + race_cat2 + age + brain2 + Yost_ACS_2006_2010_quintile, cluster="state", family="binomial")
summary(model1)
...
# significant for 15 to 39
# In_Medicaid_Flag adjusted model 15 to 39 >2010 (2010 treated as washout year)

model1 <- miceadds::glm.cluster(seer0to39_stage_exp %>% filter(age_cat3 == "15 to 39", Year_of_diagnosis >=2011), stage2 ~ In_Medicaid_Flag + race_cat2 + age + brain2 + Yost_ACS_2006_2010_quintile, cluster="state", family="binomial")
summary(model1)
...
```{r}
# define dx_year_cat
seer0to39_exp <- seer0to39%>%
  mutate(dx_year_cat = case_when(Year_of_diagnosis <2010 ~"<2010",
                                 Year_of_diagnosis >2010 ~ ">2010")) %>%
   filter(state %in% c(6,9,34,53))
table(seer0to39_exp$dx_year_cat)
# Initialize an empty data frame to store results
results <- data.frame()

# Nested loops for age_cat2, and variables
  for (i in c("0 to 14", "15 to 39")) {
    for (variable in c("In_Medicaid_Flag", "mcont66.2")) {
      for(year in c("<2010", ">2010")){
      df <- seer0to39_exp %>% filter(age_cat3 == i, dx_year_cat == year)
      formula <- as.formula(paste("Surv(fumo, can_dead) ~", variable, "+ race_ethnicity + age + Yost_ACS_2006_2010_quintile + cluster(state)"))
      res.cox2 <- coxph(formula, data = df) 
      print(paste(", age_cat2 =", i, ", and variable =", variable))
      print(summary(res.cox2))
      ...
      df2$variable <- variable
      df2$model <- "Adjusted for race/ethnicity, age, and Yost SES indicator"
      df2$year_category <- year

      # Append the new row to the results data frame
      results <- rbind(results, df2 %>% filter(term %in% c("In_Medicaid_FlagEnrolled", "mcont66.2Continuous", "mcont66.2Other discontinuous", "mcont66.2Other", "mcont66.2At diagnosis")) %>% select(term, estimate, conf.low, conf.high, age_cat, variable, year_category))
    }
    }
  }
...
```{r}
# 0 to 14
df <- seer0to39_exp %>% filter(age_cat3 == "0 to 14")
      model0 <- coxph(Surv(fumo, can_dead) ~ In_Medicaid_Flag + race_ethnicity + age + Yost_ACS_2006_2010_quintile + dx_year_cat + In_Medicaid_Flag*dx_year_cat + cluster(state), data = df) 
summary(model0) # not significant

      model1 <- coxph(Surv(fumo, can_dead) ~ mcont66.2 + race_ethnicity + age + Yost_ACS_2006_2010_quintile + dx_year_cat  + cluster(state), data = df) 
summary(model1)

      model2 <- coxph(Surv(fumo, can_dead) ~ mcont66.2 + race_ethnicity + age + Yost_ACS_2006_2010_quintile + mcont66.2*dx_year_cat + cluster(state), data = df) 
summary(model2)

lrtest(model1, model2) # not significant

# 15 to 39

df2 <- seer0to39_exp %>% filter(age_cat3 == "15 to 39")

      model0 <- coxph(Surv(fumo, can_dead) ~ In_Medicaid_Flag + race_ethnicity + age + Yost_ACS_2006_2010_quintile + dx_year_cat + In_Medicaid_Flag*dx_year_cat + cluster(state), data = df2) 
summary(model0) # not significant

      model1 <- coxph(Surv(fumo, can_dead) ~ mcont66.2 + race_ethnicity + age + Yost_ACS_2006_2010_quintile + dx_year_cat  + cluster(state), data = df2) 
summary(model1)

      model2 <- coxph(Surv(fumo, can_dead) ~ mcont66.2 + race_ethnicity + age + Yost_ACS_2006_2010_quintile + mcont66.2*dx_year_cat + cluster(state), data = df2) 
summary(model2)
